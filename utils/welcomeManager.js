...
async function sendWelcome(member) {
  ...
  try {
    const resp = await axios.post(
      ...
      {
        model: AI_MODEL,
        messages: [{ role: 'user', content: 'Welcome a new member! Keep it short & invite intros.' }],
        stream: false
      },
...
async function processPendingWelcomes(client) {
  ...
  for (const item of items) {
    ...
    if (member.roles.cache.size <= 1) continue;  // still no roles
